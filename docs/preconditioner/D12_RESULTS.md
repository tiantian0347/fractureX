...
- **✅ 步长细化验证（2026-06-17 补，堵"niter 是否粗加载步长 artifact"质疑）**：质疑——表 1b 用生产加载日程（局部化区 du=2.2e-3），maxd 单步从 0.426(step13) 跳到 0.998(step14)，整段 maxd∈(0.43,0.998) 过渡带**零采样**，会否漏掉隐藏的 niter 峰值、或 82 是粗步长虚构？**实测否定**：用 `--mode direct`（pardiso，对系数对比不敏感）在局部化窗口 `[0.083,0.094]` 把 du 加密 20×（→1.1e-4，run tag `paper_direct_h2refine`），生成 30 个 maxd∈(0.40,0.52) 的细化 checkpoint，再用 `d12_recheck.py`（restart=60/200，冷启动）测 niter 包络：

  | max_d（细化点） | niter @r60 | niter @r200 |
  |---|---|---|
  | 0.40 → 0.52（6 个均匀细化点） | **恒 7** | **恒 2** |
  | 0.994（snap 后） | 113–114 | 18 |

  两点结论：(1) **过渡带 (0.52, 0.998) 是物理 snap-through，非采样间隙**——即便 du 加密 20×，maxd 仍在 step072→073 单步从 0.517 跳到 0.994（位移控制下该平衡分支不稳定、不可达）。故表 1b 的"局部化跃迁"是**真实物理失稳**，不是步长太粗造成的人为跳变；可达 maxd 上界 ≈0.52，再往上即跳到 ≈0.994。(2) **局部化前 niter 平且极低**（maxd≤0.52 恒 7@r60 / 2@r200，与 paper_aux_h2 step013 maxd=0.426 测得的 7/2 **独立吻合**），**升高完全发生在 snap 之后**——印证表 1b "niter 与 maxd 同步、局部化跃迁触发升高"。**注**：峰值**具体数**对 maxd 距 1.0 的距离极敏感（0.994→18 vs 表 1b step20 maxd=1.000→82，restart=200），即"峰值取多大"是 step-size/逼近方式相关量；但**定性量级（局部化后 O(10–50) 有界、restart 主导）不变**。正文报量级与有界性，不把单个峰值数当不变量。〔数据 `paper_direct_h2refine`，脚本 `d12_recheck.py`，TAG 经 `D12_TAG` 切换。〕
