"""model1 SG variant: 单位方形 + 顶部拉伸 + 应变梯度弹性耦合。

与 `Model1SquareTensionCase` 相同的几何/材料/BC，只是 solver 换成
`IPFEMPhaseFieldSGSolver`，多一个 `ell_s`（应变梯度长度）参数以及
`sg_split` 开关（Ali 2024 拉伸-only 谱分解）。
"""
...
from .model1_square_tension import _default_init_mesh, _default_load_sequence


@dataclass
class Model1SGCase:
    """model1 拉伸 + strain-gradient 弹性。"""

    name: str = "ipfem_model1_sg"
    E: float = 210.0
    nu: float = 0.3
    Gc: float = 2.7e-3
    l0: float = 0.0133
    ...
    def run(
        ...
        *,
        ...
        for i in range(len(disp) - 1):
            if verbose:
                print(
                    f"[model1_sg ell_s={self.ell_s}] step {i+1}/{len(disp)-1} "
                    f"disp={disp[i+1]:.4e}"
                )
