"""model2: 单位方形 + 顶部 x 方向剪切位移 + 上下夹持。

对齐 ttthesis/code/ip_hybrid_mix/test_model2.py。

网格：手工 8 个三角形起始（同 model1，含预裂纹节点重复），uniform_refine(n)。
材料：E=210, ν=0.3, Gc=2.7e-3, l0=0.004。
BC：
  - is_disp_boundary   : y=1 上 x 方向规定位移（剪切）
  - is_dirchlet_boundary : (y=0)∪(y=1) 全固定
"""
...
@dataclass
class Model2ShearCase:
    """model2: 顶部 x 方向剪切。"""

    name: str = "ipfem_model2_shear"
    E: float = 210.0
    nu: float = 0.3
    Gc: float = 2.7e-3
    l0: float = 0.004
    ...
    def run(
        ...
        *,
        ...
        for i in range(len(disp) - 1):
            if verbose:
                print(f"[model2] step {i+1}/{len(disp)-1} disp={disp[i+1]:.4e}")
