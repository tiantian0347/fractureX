#!/usr/bin/env python3
"""D2：square（model1，I 型直裂纹拉伸）的迭代稳定性对照扫描。
...
用法: iter_stability_square.py [nx1 nx2 ...]   默认 80 120
输出: results/phasefield/_iter_stability/iter_stability_square.csv
"""
...
LOAD = 5e-3  # pre-crack tension regime
...
def main():
    ...
    for nx in nxs:
        ...
        case = SquareTensionPreCrackCase(_model=SquareMat(), nx=nx, ny=nx, debug_mesh=False)
        mesh = case.make_mesh()
        ...
        for dval in DVALS:
            ...
            for pc, r in (("none", r_none), ("jacobi", r_jac), ("ilu", r_ilu), ("aux_fast", r_aux)):
                rows.append({"case": "square", "nx": nx, "sigma": m, "d": dval, "precond": pc, **r})
                print(f"[sq nx={nx} d={dval:5.3f}] {pc:9s} niter={r['niter']:>6} "
                      f"conv={r['converged']!s:5} t={r['t_s']:.1f}s {r['note']}", flush=True)
    csvp = outdir / "iter_stability_square.csv"
