#!/usr/bin/env bash
# Foreground runner for the ip_fracture paper MMS convergence study.
# Wrap with nohup/screen/sbatch when launching in the background.
#
# Usage:
#   bash scripts/paper_ipfem/run_mms_convergence.sh
#   nohup bash scripts/paper_ipfem/run_mms_convergence.sh > /tmp/mms.log 2>&1 &
#
# Status/pid/exit files land under $FRACTUREX_PAPER_LOG_DIR (default results/logs).
set -euo pipefail
...
_rc=0
"${FRACTUREX_PYTHON}" \
  "${_REPO_ROOT}/scripts/paper_ipfem/mms_convergence.py" \
  "$@" >> "${JOB_LOG}" 2>&1 || _rc=$?
