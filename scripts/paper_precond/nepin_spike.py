...
def main() -> int:
    ...
    # The rebuilt-from-recorder case has no material model attached; recover it
    # from meta.json (material params are recorded there) so on_build works.
    import json as _json
    _meta_path = ckdir_root / "meta.json"
    _material = dict((_json.loads(_meta_path.read_text()).get("material") or {}))
    if not _material:
        print(f"[spike] ERROR: no material in {_meta_path}", file=sys.stderr)
        return 2
    discr.case.model = lambda _m=_material: _m
    print(f"[spike] material from meta.json: "
          f"lam={_material.get('lam')}, mu={_material.get('mu')}, "
          f"Gc={_material.get('Gc')}, l0={_material.get('l0')}")
